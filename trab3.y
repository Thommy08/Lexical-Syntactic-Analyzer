...
// Declarações explícitas para evitar declarações implícitas
int yylex(void);
...
void movimento(int distancia);
void voar(int alt);
void flush_movements();
...
block:
    ...
    {
        if (valid) {
            flush_movements(); // Garante que todos os movimentos sejam adicionados ao buffer
...
instruction:
    ON_COMMAND
    {
        flush_movements();
        if (estado_nave == 1)
            erro("A nave já se encontra-se ligada");
        else
        {
            estado_nave = 1;
            printf("ação(ligar)\n");
    ...
    | OFF_COMMAND
    {
        flush_movements();
        if (estado_nave == 0)
            erro("A nave já se encontra-se desligada");
        else
        {
            estado_nave = 0;
    ...
    | TAKEOFF_COMMAND
    {
        flush_movements();
        if (voo_nave == 1)
            erro("A nave já se encontra-se no ar");
        else
        {
            voo_nave = 1;
    ...
    | LAND_COMMAND
    {
        flush_movements();
         if (voo_nave == 0)
            erro("A nave já se encontra-se em terra");
        else
        {
            voo_nave = 0;
    ...
    | TURN_COMMAND
    {
        flush_movements();
        if (estado_nave == 0)
            erro("A nave deve estar ligada para girar.");

        angulo_atual = $1.value;

        if (angulo_atual < 0 || angulo_atual >= 360)
            erro("Ângulo inválido para virar (0 ou >= 360 graus).");
        ...
        {
            erro("Direção inválida. Use 'L' para esquerda ou 'R' para direita.");
        }
    }
    | MOVE_COMMAND
    {
        if (estado_nave == 0)
            erro("A nave não pode ser mover quando está desligada");
        else
        {
            distancia_atual = $1.value;
            movimento(distancia_atual);
        }
    ...
    | FLY_COMMAND
    {
        ...
        if (estado_nave == 0)
            erro("A nave não pode ser mover quando está desligada");
        if (voo_nave == 0)
            erro("A nave não pode mover se no ar sem levantar voo");
        else
            altura = $1.value;
            voar(altura);
    }
...
void movimento(int distancia) {
    ...
    if (tmp_x >= min_x && tmp_y >= min_z && tmp_x <= max_x && tmp_y <= max_y)
    {
        ...
        erro("Movimento fora dos limites impostos, as coordenadas permanecem as mesmas");
...
void voar(int alt) {
    if ((z + alt) < 0)
        erro("A nave não pode passar do limite da superfície da terra.");
    if ((z + alt) > max_z)
        erro("A nave não pode passar do limite máximo imposto");
